...
const DEFAULT_MODEL_NAME = 'mistral-small-latest';
...
export async function generateAiPlanGemini(user, metrics) {
    ...
    try {
        ...
        // ✅ FIX: Correct Gemini model name (gemini-3.5-flash does not exist)
...
export async function generateAiPlan(user, metrics) {
    if (process.env.GEMINI_API_KEY) {
        console.log('🤖 Generating plans using Gemini 1.5 Flash...');
        const plan = await generateAiPlanGemini(user, metrics);
        if (!plan.error) {
            return plan;
        }
        console.warn('⚠️ Gemini failed, falling back to Mistral... Error was: ' + plan.error);
    }

    if (process.env.MISTRAL_API_KEY) {
        console.log('🤖 Generating plans using Mistral...');
        return await generateAiPlanMistral(user, metrics);
    }

    return {
        error: 'Neither GEMINI_API_KEY nor MISTRAL_API_KEY is configured. Please set your API keys in environment variables.'
    };
}
